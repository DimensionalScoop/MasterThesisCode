...
@to_list
def format_ufloat(f):
    if type(f) == type(ufloat(1, 1)):
        f = [f]
    for i in f:
        sig_fig = -np.floor(np.log10(i.s))
        if sig_fig < 0:
            yield ("{:.0f}").format(i.n) + "+-" + ("{:.0f}").format(i.s)
        else:
            yield ("{:." + str(int(sig_fig)) + "f}").format(i.n) + "+-" + (
                "{:." + str(int(sig_fig)) + "f}"
            ).format(i.s)
...
def autofit(
    ...
    if isinstance(y[0], uncertainties.UFloat):
        ...
        params, covariance = curve_fit(
            fitFunction, x, y, p0=p0_, maxfev=MAX_FEV, sigma=weights, bounds=bounds
        )
